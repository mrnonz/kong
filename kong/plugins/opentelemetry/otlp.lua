...
local function to_pb(data)
  return assert(pb.encode("opentelemetry.proto.collector.trace.v1.ExportTraceServiceRequest", data))
end
    ...
    resource_spans = {
      {
        resource = {
          attributes = {
            ...
            { key = "service.instance.id", value = { string_value = kong.node.get_id() } },
            { key = "service.version", value = { string_value = kong.version } },
          }
        },
